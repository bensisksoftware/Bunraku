public class ClickRight{
	public static void turnRight() {
		// shut everything when turn, except when holding doll
		if (!World.satchelOpen && (Player.position != "h11sRoom") && Player.position != "h11s") {
			if (World.open) {
				Audio.startSafeShut();
				System.out.println("ClickRight.turnRight() close safe (top of class)");
				World.open = false;
			}
		}
		...
		switch (Player.position) {
			...
			case "h15w":
				...
				break;
			case "h15n":
				if (!Keyboard.typingInput && !Player.readingMessage && Player.onPhone) {
			...
			case "h17e":
				...
				break;
			case "h18s":
				if (Player.holdingRedBook || Player.holdingBlackBook || Player.holdingBlueBook && !Animation.loweringBook) {
					Item.bookPage = 0;
					Animation.loweringBook = true;
					Player.setDisableWalk(true);
					Audio.startEquip2();
			...
			case "tTVControls":
				...
				break;
			default:
				System.out.println("Click.turnRight() error");
				break;
		}
	}
}